...
def extract_contexts(
    messages_dict: dict[tuple, dict[str, Any]], channel_id: int, message_id: int, window: int = 2
) -> list[str]:
    """
    ...
    target_idx = None
    for idx, (_date, msg_id, msg) in enumerate(channel_messages):
    ...
    logger.debug(
        f"Извлечено {len(contexts)} контекстов для channel_id={channel_id}, message_id={message_id} (window={window})"
    )
...
def extract_contexts_by_sources(
    messages_dict: dict[tuple, dict[str, Any]], sources_info: list[dict[str, Any]]
) -> list[str]:
    """
    ...
    for source in sources_info:
        ...
        if channel_id is None or message_id is None:
            try:
                if "_" in doc_id:
                    ...
                    logger.warning(
                        f"Не удалось определить channel_id и message_id для doc_id={doc_id}, metadata={metadata}"
                    )
                    continue
        ...
        if key in messages_dict:
            ...
            logger.warning(
                f"Сообщение не найдено в БД: doc_id={doc_id}, channel_id={channel_id}, message_id={message_id}, metadata={metadata}"
            )

    logger.debug(f"Извлечено {len(contexts)} контекстов из {len(sources_info)} источников")
