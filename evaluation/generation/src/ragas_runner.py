...
def _print_intermediate_metrics(results: list[dict[str, Any]], current_idx: int, total: int):
    ...
    for metric in metrics:
        if metric in df.columns:
            ...
            if len(values) > 0:
                metric_values[metric] = {"mean": float(values.mean()), "std": float(values.std())}
...
async def run_evaluation_async(
    examples: list[dict[str, Any]], judge_config: dict[str, Any], use_ragas: bool = True, batch_size: int = 10
) -> pd.DataFrame:
    """
...
async def _run_ragas_evaluation_async(
    examples: list[dict[str, Any]], judge_config: dict[str, Any], batch_size: int
) -> pd.DataFrame:
    """
    ...
    for ex in examples:
        ragas_data.append(
            {"question": ex["question"], "contexts": ex["contexts"], "answer": ex["answer"], "ground_truth": ""}
        )
    ...
    try:
        ...
        evaluate_kwargs = {"dataset": ragas_df, "metrics": metrics}
    ...
    async def process_single_example(idx: int, ex: dict[str, Any]) -> dict[str, Any]:
        ...
        if result_row.get("faithfulness", 0.0) > 0.0:
            ...
            (
                ...
                off_topic_rate,
                has_error,
            ) = await score_all_metrics_async(judge_client, ex["question"], ex["answer"], ex["contexts"], cited_sources)
...
def run_evaluation(
    examples: list[dict[str, Any]], judge_config: dict[str, Any], use_ragas: bool = True
) -> pd.DataFrame:
    """
...
def _run_ragas_evaluation(examples: list[dict[str, Any]], judge_config: dict[str, Any]) -> pd.DataFrame:
    """
...
def _run_custom_evaluation(examples: list[dict[str, Any]], judge_config: dict[str, Any]) -> pd.DataFrame:
    """
