...
def load_generated_answers(generated_answers_path: Path) -> dict[int, dict[str, Any]]:
    ...
    examples_dict = {}
    with open(generated_answers_path, encoding="utf-8") as f:
...
def find_best_worst(
    results_path: str, output_dir: str, use_composite: bool = True, generated_answers_path: str = None
) -> None:
    """
    ...
    if best_full.get("answer"):
        print(
            f"\nAnswer: {best_full['answer'][:500]}..."
            if len(best_full.get("answer", "")) > 500
    ...
    if worst_full.get("answer"):
        print(
            f"\nAnswer: {worst_full['answer'][:500]}..."
            if len(worst_full.get("answer", "")) > 500
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out", type=str, default="eval/outputs", help="Директория для сохранения результатов (default: eval/outputs)"
    )
    parser.add_argument(
        "--no-composite", action="store_true", help="Использовать среднее по метрикам вместо композитной оценки"
    )
    parser.add_argument(
        "--generated-answers",
        type=str,
