...
class ServiceManager:
    ...
    def __init__(self, project_root: Path | None = None, use_docker: bool = True, auto_stop: bool = True):
        """
    ...
    def wait_for_service(self, url: str, max_wait: int = 120, check_interval: int = 2) -> bool:
        """
    ...
    def start_directly(self, service_name: str, module_path: str, port: int) -> bool:
        ...
        try:
            ...
            process = subprocess.Popen(
                cmd, cwd=self.project_root, stdout=log_handle, stderr=subprocess.STDOUT, text=True, env=env
            )
            ...
            if process.poll() is None:
                ...
                try:
                    ...
                    logger.error(f"Ошибка при чтении логов {service_name}")
                return False
    ...
    def stop_service(self, service_name: str):
        ...
        if service_name in self.processes:
            ...
            try:
                subprocess.run(
                    ["docker-compose", "stop", service_name], cwd=self.project_root, capture_output=True, check=True
                )
    ...
    def ensure_generation_service(self, url: str = "http://localhost:8022/health", wait: bool = True) -> bool:
        """
        ...
        if self.use_docker:
            ...
            success = self.start_directly("generation", "tplexity.generation.app:app", 8022)
    ...
    def ensure_services_for_inference(self, inference_endpoint: str, wait: bool = True) -> bool:
        """
        ...
        if ":8022" in inference_endpoint or "generation" in inference_endpoint.lower():
            ...
            health_urls = {"retriever": "http://localhost:8020/health", "generation": "http://localhost:8022/health"}
            ...
            if need_start:
                ...
                if self.use_docker:
                    ...
                    logger.info(
                        f"Установлены переменные окружения: RETRIEVER_API_URL={os.environ.get('RETRIEVER_API_URL')}, QWEN_BASE_URL={os.environ.get('QWEN_BASE_URL')}"
                    )

                    if "retriever" in need_start:
                        success = self.start_directly("retriever", "tplexity.retriever.app:app", 8020) and success
                    if "generation" in need_start:
                        success = self.start_directly("generation", "tplexity.generation.app:app", 8022) and success
