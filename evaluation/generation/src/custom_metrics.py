...
class JudgeClient:
        ...
        if self.provider == "qwen":
            if not QWEN_AVAILABLE:
                ...
                try:
                    ...
                    logger.info(
                        f"Инициализирован Qwen judge: model={llm_settings.qwen_model}, base_url={llm_settings.qwen_base_url}"
                    )
            ...
            if not QWEN_AVAILABLE:
                ...
                try:
                    ...
                    logger.info(
                        f"Инициализирован YandexGPT judge: model={llm_settings.yandexgpt_model}, base_url={llm_settings.yandexgpt_base_url}"
                    )
                except Exception as e:
    ...
    def _call_judge(self, prompt: str, max_retries: int | None = None) -> str:
        """
                ...
                try:
                    response = self.client.chat.completions.create(
                        ...
                        messages=[
                            {
                                "role": "system",
                                "content": "Ты — эксперт по оценке качества ответов. Всегда возвращай только валидный JSON.",
                            },
    ...
    def _call_qwen_judge(self, prompt: str, max_retries: int) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    logger.warning(
                        f"Ошибка при вызове {self.provider} judge (попытка {attempt + 1}/{max_retries + 1}): {e}"
                    )
                    continue
                else:
...
def score_relevance(judge_client: JudgeClient, question: str, answer: str) -> tuple[float, str | None, bool]:
    """
...
def score_faithfulness(judge_client: JudgeClient, contexts: list[str], answer: str) -> tuple[float, list[str], bool]:
    """
...
def score_completeness(
    judge_client: JudgeClient, question: str, answer: str, contexts: list[str]
...
async def score_all_metrics_async(
    judge_client: JudgeClient, question: str, answer: str, contexts: list[str], cited_sources: list[str]
    ...
    prompt = PROMPT_ALL_METRICS.format(question=question, answer=answer, contexts=contexts_text)
            ...
            for attempt in range(judge_client.max_retries + 1):
                try:
                    response = await judge_client.llm_client.generate(
                        messages=messages, temperature=judge_client.temperature, max_tokens=2000
                    )
                    result = judge_client._parse_json_response(response, default_score=0.0)
                    ...
                    if attempt < judge_client.max_retries:
                        logger.warning(
                            f"Ошибка при вызове {judge_client.provider} judge (попытка {attempt + 1}/{judge_client.max_retries + 1}): {e}"
                        )
                        continue
                    else:
                        logger.error(
                            f"Не удалось вызвать {judge_client.provider} judge после {judge_client.max_retries + 1} попыток: {e}"
                        )
                        return 0.0, 0.0, 0.0, 0.0, True
