...
from ragas_runner import run_evaluation_async
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for example in dataset:
            ...
            json_line = json.dumps(
                {"question": example["question"], "contexts": example["contexts"], "answer": example["answer"]},
                ensure_ascii=False,
            )
            f.write(json_line + "\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RAG Evaluation Pipeline", formatter_class=argparse.RawDescriptionHelpFormatter
    )

    parser.add_argument("--posts", type=str, required=True, help="Путь к JSON файлу с сообщениями")

    parser.add_argument("--queries", type=str, required=True, help="Путь к JSON файлу с запросами")
    ...
    parser.add_argument(
        "--inference-endpoint", type=str, default="", help="URL inference endpoint (если пусто - используется mock)"
    )
    ...
    parser.add_argument("--window", type=int, default=2, help="Размер окна для контекста (default: 2)")
    ...
    parser.add_argument("--use-ragas", action="store_true", help="Использовать Ragas для оценки (если доступен)")
    ...
    parser.add_argument(
        "--no-docker", dest="use_docker", action="store_false", help="Запускать сервисы напрямую (без docker-compose)"
    )

    parser.add_argument("--keep-services", action="store_true", help="Не останавливать сервисы после завершения")

    parser.add_argument(
        "--limit", type=int, default=None, help="Ограничить количество обрабатываемых запросов (для тестирования)"
    )

    parser.add_argument(
        "--batch-size", type=int, default=10, help="Размер батча для асинхронной обработки запросов (default: 10)"
    )

    parser.add_argument("--show-answers", action="store_true", help="Выводить ответы модели в консоль при генерации")
    ...
    if args.auto_start_services:
        service_manager = ServiceManager(use_docker=args.use_docker, auto_stop=not args.keep_services)
    ...
    inference_client = InferenceClient(
        endpoint_url=inference_endpoint, api_key=inference_api_key, timeout=inference_timeout
    )
        ...
        try:
            ...
            if sources_info:
                ...
                if inference_endpoint:
                    ...
                    documents = await get_documents_from_retriever_async(retriever_url, doc_ids, inference_timeout)

                    if documents:
                        # Используем тексты из retriever API (в том же порядке, что и doc_ids)
                        doc_id_to_text = {doc["doc_id"]: doc["text"] for doc in documents}
                        used_contexts = [
                            doc_id_to_text.get(doc_id, "") for doc_id in doc_ids if doc_id_to_text.get(doc_id)
                        ]

                        if not used_contexts:
                            logger.warning(
                                f"Не удалось получить тексты документов из retriever API для query: {question[:50]}..."
                            )
                            return None
                    else:
                    ...
                    if not used_contexts:
                        logger.warning(
                            f"Не удалось извлечь контексты из {len(sources_info)} источников для query: {question[:50]}..."
                        )
                        return None
            else:
                logger.warning(f"Generation не вернул источников для query: {question[:50]}...")
    ...
    async def process_batch(queries_batch: list[dict[str, Any]], client: InferenceClient) -> list[dict[str, Any]]:
        ...
        logger.info(
            f"Батч из {len(queries_batch)} запросов обработан за {batch_time:.2f}ms (среднее: {batch_time / len(queries_batch):.2f}ms на запрос)"
        )
    ...
    results_df = asyncio.run(
        run_evaluation_async(examples, judge_config, use_ragas=args.use_ragas, batch_size=args.batch_size)
    )
