...
class LLMClient:
    ...
    def __init__(self, base_url: str | None = None):
        ...
        logger.info(f"🔄 [generation][llm_client] Инициализирован клиент для {self.base_url}")
    ...
    async def generate(
        ...
        try:
            ...
            logger.error(f"❌ [generation][llm_client] HTTP ошибка от LLM Client API: {e.response.status_code}")
            raise
        except Exception as e:
            logger.error(f"❌ [generation][llm_client] Ошибка при запросе к LLM Client API: {e}")
            raise

    async def close(self) -> None:
        """Закрывает соединение с LLM Client API"""
        await self.client.aclose()
        logger.info("🔌 [generation][llm_client] Соединение с LLM Client API закрыто")
